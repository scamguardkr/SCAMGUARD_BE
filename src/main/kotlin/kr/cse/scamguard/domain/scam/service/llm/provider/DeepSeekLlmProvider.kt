...
class DeepSeekLlmProvider (
    @Qualifier("deepSeekChatModel") private val chatModel: OpenAiChatModel,
    ...
    override fun analyzeScam(prompt: String): LlmScamAnalysisResult {
        ...
        try {
            ...
            log.debug("DeepSeekLlm 응답: {}", response)

            // JSON 파싱
            return parseResponse(response)
        } catch (e: Exception) {
            log.error("DeepSeekLlm 분석 중 오류 발생", e)
            log.error("DeepSeekLlm 분석 중 오류 발생 - 오류 메시지 : {}", e.message)
            throw RuntimeException("OpenAI 분석 실패", e)
        }
    }

    override fun getModelName(): String = "deepseek-chat"
